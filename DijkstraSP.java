...
public class DijkstraSP {
    ...
    public DijkstraSP(EdgeWeightedGraph G, int s) {
        for (Edge e : G.edges()) {
            if (e.getTravelTime() < 0)
                throw new IllegalArgumentException("edge " + e + " has negative getTravelTime");
        }
    ...
    private void relax(Edge e) {
        ...
        if (distTo[w] > distTo[v] + e.getTravelTime()) {
            distTo[w] = distTo[v] + e.getTravelTime();
            edgeTo[w] = e;
    ...
    private boolean check(EdgeWeightedGraph G, int s) {
        ...
        for (Edge e : G.edges()) {
            if (e.getTravelTime() < 0) {
                System.err.println("negative edge getTravelTime detected");
                return false;
            }
        }
        ...
        for (int v = 0; v < G.V(); v++) {
            for (Edge e : G.adj(v)) {
                ...
                if (distTo[v] + e.getTravelTime() < distTo[w]) {
