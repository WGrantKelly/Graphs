...
public class LazyPrimMST {
    ...
    private double travelTime;       // total travelTime of MST
    ...
    private boolean check(EdgeWeightedGraph G) {
        ...
        for (Edge e : edges()) {
            totalWeight += e.getTravelTime();
        }
        if (Math.abs(totalWeight - travelTime()) > FLOATING_POINT_EPSILON) {
        ...
        for (Edge e : edges()) {
            ...
            for (Edge f : G.edges()) {
                int x = f.either(), y = f.other(x);
                if (!uf.connected(x, y)) {
                    if (f.getTravelTime() < e.getTravelTime()) {
